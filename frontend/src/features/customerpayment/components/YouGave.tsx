...
interface YouGaveProps {
    customerId?: number;
    onSuccess: () => void;
    onClose: () => void;
}

export const YouGave: React.FC<YouGaveProps> = ({ customerId, onSuccess, onClose }) => {
    // 1. Extract customer ID from URL route params as primary fallback
    const { id } = useParams<{ id: string }>();

    // 2. Safely compute numerical activeCustomerId from prop or route path
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        // Guard against invalid/missing customer ID
        ...
        try {
            await createPayment({
                ...
                paymentCategory: 'GIVEN', // "You Gave" mapping
